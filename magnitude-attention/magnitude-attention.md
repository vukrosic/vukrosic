...
1.  **Being Grouped**: Systematic redundancy means many keys $\{k_1, \dots, k_m\}$ occupy the same region in vector space. Consequently, they all produce the same or similar dot product with the query: $q \cdot k_i \approx \text{constant}$.
...
Despite the unique key being **$20\times$ more relevant** than any individual cluster member, the redundant group dominates the attention mechanism by sheer volume.
...
2.  **$\boldsymbol{\mu}$ (The Unknown Weights)**: This is a single column vector containing one weight for **every** key in the sequence: $\boldsymbol{\mu} = [\mu_1, \mu_2, \dots, \mu_N]^\top$. In the summation formula, we use the index $i$ to denote the "current key" whose constraint we are checking, and the index $j$ to iterate over all its neighbors.

3.  **$\mathbf{1}$ (The Unit Constraint)**: This is a vector of all ones. It acts as the target for every row.
...
(If you want to dive deeper into the technical mechanics, research (LLMs may help): **"Why is the Gaussian (RBF) kernel strictly positive definite?"**, **"How does the SPD property guarantee matrix invertibility?"**, and **"Numerical stability of solving linear systems with SPD matrices"**.)

With this stable foundation, we can now build the full attention mechanism.

### Key Properties (proven by Leinster, Willerton, Meckes, and others)
...
| **Information Additivity** | If two groups of tokens are totally unrelated (far apart in space), their total information is the sum of their individual parts. |
| **Diversity Limit** | The total magnitude represents the absolute maximum "variety" or diversity present in the entire set of tokens. |
...
Given keys $\{k_1, \dots, k_n\}$, we construct the similarity matrix $Z$. Unlike standard attention which uses a fixed dot product, we use a Gaussian kernel with a **learnable scale $t$**:
